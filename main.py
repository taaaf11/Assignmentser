...
async def main(page: ft.Page):
    ...
    async def show_assignment_create_dialog(_):
        async def add_assignment_control_to_page(_):
            page.assignments_list.add(
                AssignmentControl(await crt_control.get_assignment())
            )
            page.close(dialog)
        ...
        dialog = ft.AlertDialog(
            content=crt_control,
            actions=[
                ft.IconButton(ft.icons.CHECK, on_click=add_assignment_control_to_page)
            ],
        )
        page.open(dialog)
    ...
    page.floating_action_button = ft.FloatingActionButton(
        icon=ft.icons.ADD, on_click=show_assignment_create_dialog
    )
